...
type Query {
  ...
  ssn: String @secret(top: true)
    ...
    describe "when the schema contains a node with multiple repeatable-directive annotations" do
      let(:schema_idl) { <<-GRAPHQL
directive @repeatableDirective(fakeArgument: String!) repeatable on FIELD_DEFINITION

type FakeTypeDef {
  id: ID! @repeatableDirective(fakeArgument: "There are more than one of me...")
}

type Query {
  fakeQueryField: FakeTypeDef! @repeatableDirective(fakeArgument: "Cats love dogs...") @repeatableDirective(fakeArgument: "Dogs love cats...")
}
      GRAPHQL
      }
    ...
    describe "when the schema contains a node with multiple repeatable-directive annotations on ENUM_VALUE" do
      let(:schema_idl) { <<-GRAPHQL
directive @repeatableDirective(fakeArgument: String!) repeatable on ENUM_VALUE

enum FakeEnum {
  VALUE1
  VALUE2 @repeatableDirective(fakeArgument: "Cats love dogs...") @repeatableDirective(fakeArgument: "Dogs love cats...")
}
